...
public class SmoothPursuitsTask : MonoBehaviour
{
    public static string file_path = Directory.GetCurrentDirectory();
    public static string file_name = "SmoothPursuitsTask";
    ...
    public Vector3 camera_pos, initial_pos, length_and_direct;
    ...
    void Update()
    {
        ...
        time_unity = Time.time - startTime;

        if (currentStep == 0 && time_unity >= 3f)
        {
        ...
        else if (currentStep == 1 && time_unity >= 6f)
        {
            start_task = true;
            currentStep++;
        }
        else if (currentStep == 2 && time_unity >= 18f)
        {
            StopDataRecord();
#if UNITY_EDITOR
        ...
        if (start_task)
        {
            ...
            Sphere.transform.position = initial_pos + length_and_direct * offset;
        }
    }
    ...
    void SetPosesAndDirection()
    {
        ...
        length_and_direct = new Vector3(3, 0, 0);
        Camera.transform.position = camera_pos;
        Sphere.transform.position = initial_pos;
    }
